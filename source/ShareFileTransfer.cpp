...
protected:
	...
	MessageRef _msgRef;
	BMessenger _replyTo;
	volatile bool* _shutdownFlagMD5;
};
...
public:
	...
	{
		TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived start\n"));
		Message& msg = *_msgRef();
		for (HashtableIterator<String, OffsetAndPath> iter(_fileSet.GetIterator()); iter.HasData(); iter++) {
			/*if ((_shutdownFlagMD5 != NULL) && (_shutdownFlagMD5 != false)){
				break;
			}*/
			const OffsetAndPath nextValue = iter.GetValue();
			msg.AddString("files", iter.GetKey().Cstr());
			TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived File = %s\n", iter.GetKey().Cstr()));
		...
		Reply();
		TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived end\n"));
	}
...
public:
	virtual void MessageReceived(BMessage *)
	{
		TRACE_SHAREFILETRANFER(("%s::%s : MessageRef end \n",CLASSNAME, __func__));
		TRACE_SHAREFILETRANFER(("MD5ReceiveLooper::MessageReceived begin\n"));
		...
		for (int i = 0; i < numRefs; i++) {
			...
			off_t offset = 0LL;
			if ((msg->FindInt64("offsets", i, &offset) == B_NO_ERROR)
				&& (offset > 0)) {
				...
				if ((msg->FindData("md5", B_RAW_TYPE, i, (const void **) &hisDigest, &numBytes) == B_NO_ERROR)
					...
					for (size_t h=0; h<ARRAYITEMS(myDigest); h++)
						myDigest[h] = 'x';	// so I know when it hasn't been overwritten

					off_t onSuccessOffset = offset;	// save this now, as we might change it below, but need it later

					off_t readLength = 0;
					if ((msg->FindInt64("numbytes", i, (int64*)&readLength) == B_NO_ERROR)
						&& (readLength <= offset)) {
					...
					off_t retBytesHashed = 0;
					BFile tempFile;
					if ((HashFileMD5(BEntry(&_entryRefs[i], true), offset, readLength, retBytesHashed, myDigest, _shutdownFlagMD5) == B_NO_ERROR)
						&& (memcmp(hisDigest, myDigest, sizeof(myDigest)) == 0))
						uploadOffset = onSuccessOffset;
				}
...
ShareFileTransfer::ShareFileTransfer(const BDirectory& fileDir, const char* localSessionID, uint64 remoteInstallID, uint64 partialHashSize, uint32 bandwidthLimit) :
	_dir(fileDir),
	_currentFileSize(-1),
...
void
ShareFileTransfer::SetBandwidthLimit(uint32 limit)
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
...
status_t
ShareFileTransfer::InitAcceptSession(const char * remoteSessionID)
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	_banEndTime = 0;
	...
	for (int i = DEFAULT_LISTEN_PORT; i <= DEFAULT_LISTEN_PORT + LISTEN_PORT_RANGE; i++) {
		if ((ret = _mtt->PutAcceptFactory(((i < DEFAULT_LISTEN_PORT + LISTEN_PORT_RANGE) ? i : 0), factoryRef)) == B_NO_ERROR) {
			_acceptingOn = i;
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::InitAcceptSession _acceptingOn = %d\n", i));
...
ShareFileTransfer::GetTransferSessionRef()
{
	...
	if (_bandwidthLimit > 0) {
		...
		if (_uploadSession)
			workerRef()->SetOutputPolicy(pref);
		else
...
void
ShareFileTransfer::SendNotifyQueuedMessage()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	MessageRef notifyMsg = GetMessageFromPool(TRANSFER_COMMAND_NOTIFY_QUEUED);
	if (notifyMsg()) {
		if (_mtt)
			_mtt->SendMessageToSessions(notifyMsg);
...
ShareFileTransfer::MessageReceived(BMessage* msg)
{
	switch(msg->what)
	{
		...
		{
			TRACE_SHAREFILETRANFER(("%s::%s : TRANSFER_COMMAND_MD5_RECV_READ_DONE\n",CLASSNAME, __func__));
			UpdateTransferTime();	// now the clock starts ticking again...
			MD5Looper * looper;
			if ((msg->FindPointer("from", (void **) &looper) == B_NO_ERROR)
				&& (_md5Loopers.Remove(looper) == B_NO_ERROR)) {
				MessageRef msgRef;
				...
				if (numFiles > 0) {
					TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (TRANSFER_COMMAND_MD5_RECV_READ_DONE) Going..\n"));
		...
		{
			TRACE_SHAREFILETRANFER(("%s::%s : TRANSFER_COMMAND_MD5_SEND_READ_DONE\n",CLASSNAME, __func__));
			UpdateTransferTime();	// now the clock starts ticking again...
			MD5Looper * looper;
			if ((msg->FindPointer("from", (void **) &looper) == B_NO_ERROR)
				&& (_md5Loopers.Remove(looper) == B_NO_ERROR)) {
		...
		{
			TRACE_SHAREFILETRANFER(("%s::%s : MUSCLE_THREAD_SIGNAL\n",CLASSNAME, __func__));
			if (_mtt) {
				uint32 code;
				MessageRef next;
				while((_mtt) && (_mtt->GetNextEventFromInternalThread(code, &next) >= 0)) {
					switch(code)
					{
						...
						{
							TRACE_SHAREFILETRANFER(("%s::%s : MTT_EVENT_SESSION_DISCONNECTED\n",CLASSNAME, __func__));
							...
							if ((_currentFileBytesDone < _currentFileSize)
								|| (_currentFileIndex < _origFileSetSize))
								_errorOccurred = true;
							else {
						...
						case MTT_EVENT_INCOMING_MESSAGE:
							TRACE_SHAREFILETRANFER(("%s::%s : MTT_EVENT_INCOMING_MESSAGE\n",CLASSNAME, __func__));
							MessageReceived(next);
						break;

						case MTT_EVENT_OUTPUT_QUEUES_DRAINED:
							TRACE_SHAREFILETRANFER(("%s::%s : MTT_EVENT_OUTPUT_QUEUES_DRAINED\n",CLASSNAME, __func__));
							if (_uploadSession) {
...
uint32
ShareFileTransfer::CalculateChecksum(const uint8 * data, size_t bufSize) const
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	uint32 sum = 0L;
...
ShareFileTransfer::DoUploadAux()
{
	...
	if (_mtt) {
		if (_currentFileBytesDone >= 0) {
			...
			if ((msg())
				&& (msg()->AddData("data", B_RAW_TYPE, NULL, bufferSize) == B_NO_ERROR)
				...
				if (numBytes > 0)  {
					...
					if (_mungeMode != MUNGE_MODE_NONE) {
						...
						if (unknownMungeMode == false)
							msg()->AddInt32("mm", _mungeMode);	// so remote peer will know how we're encoding the data
					}
					...
					if (_transferStamps.GetNumItems() > MAX_TRANSFER_STAMP_QUEUE_SIZE)
						_transferStamps.RemoveHead();
			...
			if (_fileSetIter.HasData()) {
				...
				if (_currentFile.SetTo(&_currentFileEntry, B_READ_ONLY) == B_NO_ERROR) {
					...
					if ((header())
						...
						if (_currentFileBytesDone > 0LL)
							header()->AddInt64("beshare:StartOffset", _currentFileBytesDone);	// resume-download mode!

						char attrName[B_ATTR_NAME_LENGTH];
						BNodeInfo ni(&_currentFile);
						if ((ni.InitCheck() == B_NO_ERROR)
							&& (ni.GetType(attrName) == B_NO_ERROR)) {
...
ShareFileTransfer::MessageReceived(const MessageRef& msgRef)
{
	...
	switch(msg->what)
	{
		case TRANSFER_COMMAND_REJECTED:
		{
			...
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_REJECTED) \n"));
		...
		TRACE_SHAREFILETRANFER(("%s::%s : \n",CLASSNAME, __func__));
		TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_NOTIFY_QUEUED) \n"));
			if (_uploadSession == false) {
		...
		{
			TRACE_SHAREFILETRANFER(("%s::%s : \n",CLASSNAME, __func__));
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_PEER_ID) \n"));
			if (msg->FindString("beshare:FromUserName", _remoteUserName) == B_NO_ERROR)
				_displayRemoteUserName = SubstituteLabelledURLs(_remoteUserName).Trim();
		...
		case TRANSFER_COMMAND_FILE_LIST:
		{
			...
			if ((_uploadSession)
				...
				int32 mm;
				if (msg->FindInt32("mm", &mm) == B_NO_ERROR)
					_mungeMode = mm;
				...
				if (((ShareWindow *)Looper())->IsScanningShares())
		...
		{
			TRACE_SHAREFILETRANFER(("%s::%s : \n",CLASSNAME, __func__));
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_FILE_HEADER) \n"));
			...
			if ((_uploadSession == false)
				...
				{
					String path;
					if ((((ShareWindow*)Looper())->GetRetainFilePaths())
						&& (msg->FindString("beshare:Path", path) == B_NO_ERROR)) {
						CheckPath(path);
						...
						while((nextPart = tok.GetNextToken()) != NULL) {
							...
							if (subDir.InitCheck() == B_NO_ERROR)
								...
								if (dlDir.CreateDirectory(nextPart, &subDir) == B_NO_ERROR)
									dlDir = subDir;
								else
				...
				bool append = false;
				if ((msg->FindInt64("beshare:StartOffset", &startByte) == B_NO_ERROR)
					&& (startByte > 0LL)) {
					...
					if ((_currentFile.SetTo(&existingFileEntry, B_READ_WRITE) == B_NO_ERROR)
						&& (_currentFile.GetSize(&curFileSize) == B_NO_ERROR)
						&& (curFileSize == startByte))
						append = true;
				}

				if (((append)
					|| (GenerateNewFilename(dlDir, existingFileEntry, 15) == B_NO_ERROR))
					...
					TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_FILE_HEADER) _currentFileName=%s\n", _currentFileName.Cstr()));
				...
				for (MessageFieldNameIterator iter(msg->GetFieldNameIterator()); iter.HasData(); iter++) {
					...
					if ((ni.InitCheck() == B_NO_ERROR)
						&& (msg->FindString("beshare:Kind", &mimeString) == B_NO_ERROR)) {
...
void
ShareFileTransfer::SharesScanComplete()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	...
	if (msg) {
		...
		for (int i=0; (msg->FindString("files", i, &nextFile) == B_NO_ERROR); i++) {
			String nextPath;
			if (msg->FindString("beshare:Path", i, nextPath) == B_NO_ERROR)
				CheckPath(nextPath);
...
ShareFileTransfer::GenerateNewFilename(const BDirectory & dir, BEntry & entry, uint32 count) const
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	if (count == 0)
		return B_ERROR;	// avoid infinite recursion on read-only media
	...
	if (fileExists) {
		...
		if (entry.GetName(buf) == B_NO_ERROR) {
			...
			if (GenerateNewFilename(dir, entry, count-1) != B_NO_ERROR)
				return B_ERROR;
		} else
			return B_ERROR;
	}
...
void
ShareFileTransfer::AbortSession(bool error, bool forceNoRestart, bool notifyWindow)
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	if ((error == false)
		|| (forceNoRestart))
...
void
ShareFileTransfer::DrawText(BView * lv, const BRect & r, const BBitmap * b, const char * text, float y, float iconBottom)
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	lv->DrawString(text, BPoint(r.left+2.0f+(((b)
		&& ((y-_fontAscent)<=iconBottom))?(b->Bounds().Width()+6.0f):2.0f), y+r.top));
}
...
ShareFileTransfer::DrawItem(BView* lv, BRect itemRect, bool /*complete*/)
{
	...
	if ((win == NULL)
		|| (itemRect.Width() <= 0)
		|| (itemRect.Height() <= 0))
		return; // dunno why this is necessary, but it is...

	BView * doubleBufferView = win->GetDoubleBufferView();
	if (doubleBufferView == NULL)
		return;
	...
	if (doubleBufferBitmap->Lock()) {
		...
		BRect fillRectLeft(colorRect);
		{
			...
			{
				_animShift += 2.0f;
				if (_animShift > ANIM_RECT_SPACING*2.0f)
					_animShift -= (ANIM_RECT_SPACING*2.0f);
				...
				if (_md5Loopers.GetNumItems() > 0)
					...
					if (_transferStamps.GetNumItems() > 2) {
						...
						if (elapsedTime > 0) {
							...
							for (int32 i=_transferStamps.GetNumItems()-1; i>=0; i--)
								bytesInQueue += _transferStamps[i].GetNumBytes();
...
String
ShareFileTransfer::GetFileSizeDataString(int64 bd, int64 fs) const
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	if (fs > 0) {
...
void
ShareFileTransfer::BeginTransfer()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	if ((_beginTransferEnabled)
		&&(_isWaitingOnLocal)) {
...
void
ShareFileTransfer::OpenCurrentFileFolder()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	node_ref tempRef;
...
void
ShareFileTransfer::UpdateTransferTime()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
...
void
ShareFileTransfer::ResetTransferTime()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
...
void
ShareFileTransfer::RestartSession()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	...
	TRACE_SHAREFILETRANFER(("%s::%s : _currentFileName = reset = ""\n",CLASSNAME, __func__));
	...
	if (_remoteHostName.Length() > 0)
		InitConnectSession(_remoteHostName(), _remotePort, _remoteIP, _remoteSessionID());
	else
		InitAcceptSession(_remoteSessionID());
}
...
const char*
ShareFileTransfer::_GetFileName()
{
	TRACE_SHAREFILETRANFER(("%s::%s : begin\n",CLASSNAME, __func__));
	...
	// Still nothing?	Get the first name from the orig set then, if possible
	if (_origFileSet.GetIterator().HasData())
