...
{
	TRACE_BESHARENETCLIENT(("ShareNetClient::ConnectToServer begin\n"));
	...
	if ((_mtt->StartInternalThread() == B_NO_ERROR)
		&& (_mtt->AddNewConnectSession(serverName, port) == B_NO_ERROR)) {
			TRACE_BESHARENETCLIENT(("ShareNetClient::ConnectToServer true\n"));
			UpdateEncoding();
	...
	TRACE_BESHARENETCLIENT(("ShareNetClient::ConnectToServer end\n"));
}
...
//	msg->PrintToStream();
	switch(msg->what) {
		...
		{
			TRACE_BESHARENETCLIENT(("ShareNetClient::MessageReceived (TRANSFER_COMMAND_REJECTED)\n"));
			const char * from;
							...
							case USER_NAME_DEPTH: 
							{
								...
								if (strncmp(nodeName, "name", 4) == 0) {
									...
									if (pmsg->FindString("name", &name) == B_NO_ERROR) {
										...
										bool sph = false;
										TRACE_BESHARENETCLIENT(("ShareNetClient::MessageReceived PR_RESULT_DATAITEMS, found it, name = %s\n", name));
