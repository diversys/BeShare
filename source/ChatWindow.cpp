...
ChatWindow::ChatWindow(BRect defRect, const char * title, window_look look, window_feel feel, uint32 flags)
	:
	BWindow(defRect, title, look, feel, flags)
	, _isInitialized(false)
	, _isScrolling(false)
	, _fontSize(0.0f)
	, _chatText(NULL)
	, _customColorsEnabled(true)
	, _logFile(NULL)
{
...
void
ChatWindow::LogHelp(const char * cmd, int tokenID, int descID, ChatWindow * optEchoTo)
{
	String s("	/");
	s += cmd;
	s += " ";
...
{
public:
	PasteMessageFilter(uint32 command, BTextControl * control)
	:
	BMessageFilter(B_PASTE)
	, _command(command)
	, _control(control)
	{/* empty */}
...
String
ChatWindow::ProcessURL(const String & url) const
{
	String next(url);
...
int32
ChatWindow::FindMatchingBracket(const String & s, uint32 & count) const
{
...
void
ChatWindow::InsertChatText(LogDestinationType dest, const char * t, int textLen, text_run_array * optStyle)
{
	switch(dest)
	{
...
void
ChatWindow::InsertChatTextAux(const char * text, int textLen, text_run_array * optStyle)
{
	if (text) {
...
void
ChatWindow :: InsertDroppedText(const char * u)
{
...
void
ChatWindow::DoBeep(const char * which) const
{
...
void
ChatWindow::SetFontSize(const String & cmdString)
{
...
void
ChatWindow::SetFont(const String & fontString, bool doLog)
{
...
bool
ChatWindow::ColorsDiffer(const rgb_color & c1, const rgb_color & c2) const
{
...
void
ChatWindow::SetViewColor(BView * view, const rgb_color & c)
{
...
void
ChatWindow::UpdateTextViewColors(BTextView * view)
{
	BFont font;
	uint32 mode;
	rgb_color fontColor;
...
BView*
ChatWindow::AddBorderView(BView * v)
{
...
status_t
ChatWindow::UISettingsChanged(const BMessage* /*changes*/, uint32 /*flags*/)
{
	// Patch up the default colors to match the system defaults
	_defaultColors[COLOR_BORDERS]		= ui_color(B_PANEL_BACKGROUND_COLOR);
	_defaultColors[COLOR_BG]			  = ui_color(B_DOCUMENT_BACKGROUND_COLOR);
